<template>
    <div class=" h-screen container mx-auto ">
        ...
        <div class="grid grid-cols-1 flex px-2">
            <RestaurantSettings v-if="selectedRestaurant!=='' && isShowRestaurantEditPanel"
                                :restaurant="selectedRestaurant"
                                @close="closeEditPanel"
            />
            <OrderManageParent v-else-if="isShowOrderManagementPanel"
                               :restaurant="selectedRestaurant"
                               @close="closeOrderManagementPanel"
            />
            <div v-else>
                <button class="bg-primary-700 text-white rounded-md p-2 m-2 w-full hover:bg-primary-900 "
                        @click="isShowAddRestaurantDialog = !isShowAddRestaurantDialog"
                >Yeni Restoran Ekle
                </button
                >
                    ...
                    <ul class=" flex flex-col w-full">
                        ...
                        >
                            <div class="w-full grid grid-cols-10">
                                <div @click="showOrderManagementPanel(restaurant)" class="col-span-8 uppercase text-primary-700 font-bold flex-col flex justify-center">
                                    {{ restaurant.name }}
                                </div>
                                <div>
                                    <button
                                            ...
                                            @click="editRestaurant(restaurant)">Düzenle
                                    </button>
                                </div>
                                <div>
                                    <button
                                            :key="restaurant.id"
                                            class="rounded border-2 p-2 ml-1 border-sunset-orange-700 w-full text-sunset-orange-700 hover:bg-sunset-orange-700 hover:text-white"
                                            @click="deleteRestaurant(restaurant.id)">Sil
                                    </button>
                                </div>
                            </div>
                        </li>
                ...
                <add-restaurant-dialog
                        :dialog="isShowAddRestaurantDialog"
                        :user-id="userId"
                        @close="isShowAddRestaurantDialog=false"
                        @save="addRestaurant"
                />
            </div>
        </div>
...
import OrderManageParent from "./order/OrderManageParent.vue";
...
export default {
    ...
    data() {
        return {
            restaurants: [],
            isShowAddRestaurantDialog: false,
            selectedRestaurant: {},
            isShowRestaurantEditPanel: false,
            isShowOrderManagementPanel: false,
        };
    },
    methods: {
        fetchRestaurants() {
        ...
        addRestaurant(restaurant) {
            ...
            this.isShowAddRestaurantDialog = false;
        ...
        editRestaurant(restaurant) {
            ...
            this.isShowRestaurantEditPanel = true;
        },
        closeEditPanel() {
            this.isShowRestaurantEditPanel = false;
            this.selectedRestaurant = '';
        },
        closeOrderManagementPanel() {
            this.isShowOrderManagementPanel = false;
            this.selectedRestaurant = '';
        },
        showOrderManagementPanel(restaurant) {
            this.selectedRestaurant = restaurant;
            this.isShowOrderManagementPanel = true;
        },
